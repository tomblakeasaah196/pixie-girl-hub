    ...
    <div className="relative border-t hairline" ref={ref}>
      {open && (
        <div className={cn("absolute z-[65] mb-2 bottom-full p-2 rounded-[15px] dropglass animate-fade-in", collapsed ? "left-3 w-[230px]" : "left-3 right-3")}>
          ...
          {ITEMS.map((it) => (
            <button key={it.key} className="w-full flex items-center gap-[11px] p-[9px_10px] rounded-[10px] text-[13px] font-medium text-text-muted hover:bg-text-primary/[0.06] hover:text-text-primary transition-all">
              <it.icon className="w-4 h-4" />
              {it.label}
            </button>
          ))}
          <div className="h-px bg-line/30 my-1.5 mx-1" />
          <button onClick={signOut} className="w-full flex items-center gap-[11px] p-[9px_10px] rounded-[10px] text-[13px] font-medium text-danger hover:bg-danger/10 transition-all">
