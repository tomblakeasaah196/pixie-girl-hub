...
jest.mock("../../../src/modules/catalogue/catalogue.events", () => ({ emit: jest.fn() }));
jest.mock("../../../src/shared/documents/documents.service", () => ({ store: jest.fn() }));
...
describe("computeEffective (styled retail = anchor + colour + size premiums)", () => {
  test("anchor + colour premium + size premium", () => {
    expect(
      computeEffective({ anchor: 100000, colour_premium: 5000, size_premium: 15000, override: null }),
    ).toBe(120000);
  });

  test("zero premiums → anchor unchanged (every colour same price by default)", () => {
    expect(
      computeEffective({ anchor: 90000, colour_premium: 0, size_premium: 0, override: null }),
    ).toBe(90000);
  });

  test("an explicit override wins over the computed price", () => {
    expect(
      computeEffective({ anchor: 100000, colour_premium: 5000, size_premium: 15000, override: 99000 }),
    ).toBe(99000);
  });

  test("override of 0 is honoured (not treated as unset)", () => {
    expect(
      computeEffective({ anchor: 100000, colour_premium: 5000, size_premium: 15000, override: 0 }),
    ).toBe(0);
  });

  test("no anchor yet → null (price not set), unless overridden", () => {
    expect(
      computeEffective({ anchor: null, colour_premium: 5000, size_premium: 15000, override: null }),
    ).toBeNull();
    expect(
      computeEffective({ anchor: null, colour_premium: 0, size_premium: 0, override: 80000 }),
    ).toBe(80000);
  });
...
describe("colourShort (deterministic, collision-safe SKU code)", () => {
  test("derives an UPPERCASE short from the name + colour id tail", () => {
    expect(colourShort({ name: "Dark Copper", colour_id: "a1b2c3d4-aa-bb" })).toBe("DARBB");
  });
  ...
  test("two same-prefix names stay distinct via the id tail", () => {
    const a = colourShort({ name: "Natural Black", colour_id: "1111-2222-3333" });
    const b = colourShort({ name: "Natural Brown", colour_id: "4444-5555-6666" });
    expect(a).not.toBe(b);
  });
