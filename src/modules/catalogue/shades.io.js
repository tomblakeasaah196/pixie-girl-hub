...
const SHADE_COLUMNS = [
  ...
  {
    header: "Visible on website?",
    key: "is_visible_storefront",
    width: 18,
    list: ["yes", "no"],
  },
];
...
async function exportShades({ brand }) {
  ...
  const rows = shades.map((s) => ({
    ...
    is_visible_storefront: yn(s.is_visible_storefront),
  }));
  return buildWorkbook({
...
async function importShades({ brand, user, request_id, buffer }) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const input = {
      ...
      is_visible_storefront: bool(raw["Visible on website?"]) ?? true,
    };
    try {
