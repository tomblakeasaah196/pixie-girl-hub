...
async function findById({ client, brand, id }) {
  const sql = `SELECT * FROM ${tableFor(brand)} WHERE retail_partners_id = $1 LIMIT 1`;
  // NOTE: replace `retail_partners_id` with the actual PK name for this table
  const exec = client ? client.query.bind(client) : query;
