...
function tableFor(brand) {
  ...
  return `${brand}.intercompany_transactions`;
}
...
async function findById({ client, brand, id }) {
  const sql = `SELECT * FROM ${tableFor(brand)} WHERE intercompany_transactions_id = $1 LIMIT 1`;
  // NOTE: replace `intercompany_transactions_id` with the actual PK name for this table
  const exec = client ? client.query.bind(client) : query;
