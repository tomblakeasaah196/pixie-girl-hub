...
> **The phone tier (≤767px) is frozen.** Every desktop change must be *additive
> at the desktop tier only*. If a change could alter rendering at ≤767px, it is
> wrong.
...
- **Never** edit an *unprefixed* class that currently styles a phone, and
...
`useIsDesktop()` is the structural switch. Use it for *layout-shape* decisions
(split vs stacked); use `lg:`/`xl:` classes for *styling* decisions.
...
- **Content container** — `AppShell` centers page content at
  ...
  own page-level `max-w` unless a screen needs to be *narrower* (then also add
