...
function tableFor(brand) {
  ...
  return `${brand}.dashboard_configs`;
}
...
async function findById({ client, brand, id }) {
  const sql = `SELECT * FROM ${tableFor(brand)} WHERE dashboard_configs_id = $1 LIMIT 1`;
  // NOTE: replace `dashboard_configs_id` with the actual PK name for this table
  const exec = client ? client.query.bind(client) : query;
