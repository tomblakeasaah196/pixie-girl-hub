...
import { PROPOSAL_STATUS_META, PROPOSAL_STATUS_TABS, fmtDate, marginTone, fmtPct } from "./constants";
import type { Proposal } from "./types";

export function ProposalsTab({ canApprove }: { canApprove: boolean }) {
  const [status, setStatus] = useState<string>("");
  const [openId, setOpenId] = useState<string | null>(null);
  const { data, isLoading, isError, refetch } = useProposals(status || undefined);
  ...
  const cols: Column<Proposal>[] = [
    {
      ...
      header: "#",
      width: "120px",
      render: (p) => <span className="font-mono text-[11px] text-accent-glow">{p.proposal_number}</span>,
    },
    {
      key: "title",
      header: "Proposal",
      render: (p) => (
        <div>
          <div className="font-semibold text-[13px] truncate max-w-[280px]">{p.title}</div>
          {p.description && (
            <div className="text-[11px] text-text-faint truncate max-w-[280px]">{p.description}</div>
          )}
        </div>
      ),
    },
    {
      ...
      align: "right",
      width: "90px",
      render: (p) => <span className="font-mono text-xs">{p.variants_count}</span>,
    },
    {
      key: "status",
      header: "Status",
    ...
    },
    {
      ...
      header: "Created",
      width: "110px",
      render: (p) => <span className="text-text-faint text-xs">{fmtDate(p.created_at)}</span>,
    },
  ];
    ...
    <div className="space-y-4">
        ...
        {PROPOSAL_STATUS_TABS.map((t) => (
          <button
            ...
            className={cn(
              ...
              status === t.value ? "bg-accent-deep text-[#F4E9D9]" : "text-text-muted hover:text-text-primary",
            )}
          >
            {t.label}
          </button>
      ...
      <DataTable
        ...
        empty={{
          ...
          title: "No proposals",
          message: "Price changes over the instant-apply threshold appear here for CEO approval.",
        }}
      />
            ...
            {isPending && (
              <>
                ...
                <Button
                  ...
                  onClick={() => approve.mutate(p.proposal_id, { onSuccess: close })}
                >
      ...
      {isLoading || !p ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <Pill tone={PROPOSAL_STATUS_META[p.status].tone}>{PROPOSAL_STATUS_META[p.status].label}</Pill>
            <span className="text-[12px] text-text-faint">{p.variants_count} variant{p.variants_count === 1 ? "" : "s"}</span>
          </div>
          {p.description && <p className="text-[13px] text-text-muted">{p.description}</p>}
          {p.rejection_reason && (
          ...
          {action ? (
            <div className="rounded-[12px] border border-line p-3 space-y-2">
              <div className="text-[12.5px] font-semibold capitalize">{action} reason</div>
              <textarea
                value={reason}
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    mut.mutate({ id: p.proposal_id, reason: reason.trim() || undefined }, { onSuccess: close });
                  }}
                >
                  Confirm {action}
                </Button>
              ...
              <tbody>
                {(p.results ?? []).map((r) => (
                  <tr key={r.result_id} className="border-b hairline last:border-0">
                    <td className="py-2 px-3 font-mono text-[10.5px] text-text-faint">{r.variant_id.slice(0, 8)}</td>
                    ...
                    <td className="py-2 px-3 text-right">
                      <MoneyText ngn={Number(r.proposed_price_ngn)} className="text-[12px]" />
                    </td>
                    <td className="py-2 px-3 text-right">
                      <span className="inline-flex items-center gap-1">
                        <Pill tone={marginTone(r.proposed_margin_pct)}>{fmtPct(r.proposed_margin_pct)}</Pill>
                        {r.floor_breached && <span className="text-danger text-[10px]">floor</span>}
                      </span>
                    </td>
                  </tr>
                ))}
