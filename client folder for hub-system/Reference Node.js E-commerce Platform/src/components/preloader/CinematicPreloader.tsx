...
import logoLight from "@/assets/faitlyn-logo-cream.png.asset.json";
 ...
 *  1. Logo strokes fade up + gold shimmer sweeps across the wordmark
 *  2. A circular gold "iris" expands from the logo center, wiping the page in
 */
export function CinematicPreloader() {
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => setPhase(1), reduceMotion ? 100 : 700);   // shimmer
    const t2 = setTimeout(() => setPhase(2), reduceMotion ? 250 : 1300);  // iris
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          {/* Iris wipe — circular reveal that expands past the viewport */}
          <motion.div
          ...
          <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
            ...
            >
              <img
                ...
                draggable={false}
              />
              {/* Gold shimmer sweep, masked to the logo silhouette */}
              {!reduceMotion && (
                <motion.div
                  aria-hidden
                  className="absolute inset-0 pointer-events-none"
                  style={{
                    WebkitMaskImage: `url(${logoLight.url})`,
                    maskImage: `url(${logoLight.url})`,
                    WebkitMaskSize: "100% 100%",
                    maskSize: "100% 100%",
                    WebkitMaskRepeat: "no-repeat",
                    maskRepeat: "no-repeat",
                    background:
                      "linear-gradient(110deg, transparent 30%, rgba(247,230,200,0.95) 50%, transparent 70%)",
                    mixBlendMode: "screen",
                  }}
                  initial={{ x: "-120%" }}
