...
async function createAccount({ brand, user, request_id, input }) {
  return transaction(async (client) => {
    ...
    if (duplicate) {
      throw new ConflictError("An active factory account already exists for this supplier.");
    }
    ...
    await A(brand, user?.user_id, "factory.account.create", "factory_account", account.account_id, { account_name: account.account_name }, request_id);
    return account;
  });
}
...
async function updateAccount({ brand, user, request_id, id, input }) {
  return transaction(async (client) => {
    ...
    const account = await repo.updateAccount({ client, brand, id, data: input });
    await A(brand, user?.user_id, "factory.account.update", "factory_account", id, input, request_id);
    return account;
  });
}
...
async function addLedgerEntry({ brand, user, request_id, account_id, input }) {
  return transaction(async (client) => {
    ...
    if (!account) throw new NotFoundError("Factory account not found.");
    if (!account.is_active) throw new AppError({ code: "ACCOUNT_INACTIVE", httpStatus: 409, userMessage: "This factory account is no longer active." });
    ...
    if (account.credit_alert_threshold && entry.running_balance >= account.credit_alert_threshold) {
    ...
    await A(brand, user?.user_id, "factory.ledger.create", "factory_account_ledger", entry.entry_id, { account_id, entry_type: input.entry_type, amount_base: entry.amount_base }, request_id);
    return entry;
  });
}

async function listLedgerEntries({ brand, account_id, limit, offset }) {
  const account = await repo.getAccount({ client: null, brand, id: account_id });
  if (!account) throw new NotFoundError("Factory account not found.");
  return repo.listLedgerEntries({ client: null, brand, account_id, limit, offset });
}

async function reconcileEntries({ brand, user, request_id, account_id, entry_ids }) {
  return transaction(async (client) => {
    const account = await repo.getAccount({ client, brand, id: account_id });
    if (!account) throw new NotFoundError("Factory account not found.");
    const updated = await repo.reconcileEntries({ client, brand, entry_ids });
    await A(brand, user?.user_id, "factory.ledger.reconcile", "factory_account_ledger", account_id, { entry_ids }, request_id);
...
async function createShipment({ brand, user, request_id, input }) {
  return transaction(async (client) => {
    const account = await repo.getAccount({ client, brand, id: input.account_id });
    if (!account) throw new NotFoundError("Factory account not found.");
    ...
    if (input.courier_fee_original && parseFloat(input.courier_fee_original) > 0) {
    ...
    await A(brand, user?.user_id, "factory.shipment.create", "factory_shipment", shipment.shipment_id, { shipment_ref }, request_id);
...
async function listShipments({ brand, account_id, status, limit, offset }) {
  return repo.listShipments({ client: null, brand, account_id, status, limit, offset });
}
...
async function advanceShipment({ brand, user, request_id, id, input }) {
  return transaction(async (client) => {
    ...
    const shipment = await repo.advanceShipment({ client, brand, id, data: input });

    await A(brand, user?.user_id, "factory.shipment.advance", "factory_shipment", id, { status: input.status }, request_id);
...
module.exports = {
  ...
  createShipment, getShipment, listShipments, advanceShipment,
};
