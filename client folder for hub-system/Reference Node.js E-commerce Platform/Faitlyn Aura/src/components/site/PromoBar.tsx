...
/**
 * Scrolling promo strip. Duplicates the row to make a seamless loop and
 * pauses on hover so people can actually read the offers.
 */
        ...
        {loop.map((it, i) => {
          ...
          return (
            <span key={i} className="flex items-center gap-10 px-10">
              {it.to ? (
                <Link to={it.to} className="hover:text-cream transition-colors">
                  {content}
                </Link>
              ) : (
                content
