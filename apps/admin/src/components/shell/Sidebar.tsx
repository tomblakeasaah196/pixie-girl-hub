...
export function Sidebar() {
  ...
  const activeInMore = more.some((k) =>
    location.pathname.startsWith(MODULE_BY_KEY[k]!.route),
  );
  ...
  return (
    <>
      ...
      >
        ...
        <Link
          to="/"
          className="flex items-center gap-3 p-[18px] border-b hairline shrink-0 group"
        >
          {logoUrl ? (
            <img
              src={logoUrl}
          ...
          {!collapsed && (
            <span className="font-display text-[19px] tracking-wide leading-tight">
              ...
              {tail && (
                <>
                  {" "}
                  <span className="text-accent-glow">{tail}</span>
                </>
              )}
              {tagline && (
                <span className="block font-body text-[8.5px] tracking-[0.22em] uppercase text-text-faint font-semibold mt-0.5">
                  {tagline}
                </span>
              )}
            </span>
          ...
          {!isDesktop && (
            <button
              ...
              className="ml-auto p-2 text-text-faint hover:text-text-primary"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <nav
          ref={navRef}
          className="flex-1 overflow-y-auto p-[8px_10px_16px] scroll-smooth"
        >
          {!collapsed && (
            <div className="micro p-[14px_12px_6px]">Priority</div>
          )}
          {top.map(item)}
          <button
            onClick={() => setMoreOpen((o) => !o)}
            className={cn(
              "flex items-center gap-3 w-[calc(100%_-_4px)] p-[10px_12px] m-[4px_2px] rounded-[11px] text-[13px] font-semibold text-text-faint hover:text-text-primary hover:bg-text-primary/[0.04] transition-all",
              collapsed && "justify-center",
            )}
          >
            {showMore ? (
              <ChevronUp className="w-[18px] shrink-0" />
            ) : (
              <ChevronDown className="w-[18px] shrink-0" />
            )}
          ...
          {showMore &&
            groups.map(({ g, items }) => (
              <div key={g}>
                {!collapsed && (
                  <div className="micro p-[14px_12px_6px]">
                    {GROUP_LABELS[g]}
                  </div>
                )}
                {items.map(item)}
        ...
        {isDesktop && (
          ...
          >
            <ChevronLeft
              className={cn(
                "w-3.5 h-3.5 transition-transform duration-300 ease-brand",
