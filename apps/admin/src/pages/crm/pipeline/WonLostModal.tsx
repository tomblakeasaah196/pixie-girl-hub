...
export function WonLostModal({ deal, onClose }: Props) {
  ...
  const INTENTS: {
  ...
  }[] = [
    {
      ...
      icon: Trophy,
      color: "text-success",
      bg: "bg-success/[0.08] border-success/25 hover:bg-success/[0.14]",
    },
    {
      key: "lost",
      label: "Mark Lost",
      icon: XCircle,
      color: "text-danger",
      bg: "bg-danger/[0.08] border-danger/25 hover:bg-danger/[0.14]",
    },
    {
      key: "on_hold",
      label: "Put On Hold",
      icon: PauseCircle,
      color: "text-warn",
      bg: "bg-warn/[0.08] border-warn/25 hover:bg-warn/[0.14]",
    },
  ];
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-sm lg:max-w-md glass rounded-[22px] p-5 shadow-glass">
        ...
        <div className="text-[12px] text-text-faint mb-4 truncate">
          {deal.title}
        </div>
        ...
        {!intent ? (
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2 p-2.5 rounded-[10px] bg-text-primary/[0.04] border hairline">
              ...
              {intent === "on_hold" && (
                <PauseCircle className="w-4 h-4 text-warn" />
              )}
            ...
            {intent === "won" && (
              <p className="text-[12px] text-text-muted">
                This deal will be marked as won. You can then link it to a sales
                order from the deal detail page.
              </p>
            )}
            ...
            {intent === "on_hold" && (
              <p className="text-[12px] text-text-muted">
                The deal will be cancelled/paused. You can reopen it at any time
            ...
            {setStatus.isError && (
              <p className="text-[11.5px] text-danger text-center">
