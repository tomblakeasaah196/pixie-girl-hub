...
async function findById({ client, brand, id }) {
  const sql = `SELECT * FROM ${tableFor(brand)} WHERE ai_feature_flags_id = $1 LIMIT 1`;
  // NOTE: replace `ai_feature_flags_id` with the actual PK name for this table
  const exec = client ? client.query.bind(client) : query;
