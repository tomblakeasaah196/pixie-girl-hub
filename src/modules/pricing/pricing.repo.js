...
async function findById({ client, brand, id }) {
  const sql = `SELECT * FROM ${tableFor(brand)} WHERE pricing_rules_id = $1 LIMIT 1`;
  // NOTE: replace `pricing_rules_id` with the actual PK name for this table
  const exec = client ? client.query.bind(client) : query;
