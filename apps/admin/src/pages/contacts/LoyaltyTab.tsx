...
export function LoyaltyTab({ contactId, contactName }: Props) {
  ...
  const { data: summary, isLoading: summaryLoading } =
    useContactSummary(contactId);
  const { data: churnScores = [], isLoading: churnLoading } =
    useChurnScores(contactId);
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Tier card */}
      <div
        className={`p-4 rounded-[14px] border hairline ${tierConfig.bgColor}`}
      >
        ...
        {nextTier && nextThreshold && (
          <div>
            <div className="flex justify-between mb-1">
              ...
              <span className="text-[10.5px] text-text-faint">
                {TIER_CONFIG[nextTier].icon} {TIER_CONFIG[nextTier].label} at{" "}
            ...
            <p className="text-[10.5px] text-text-faint mt-1">
              {(nextThreshold - points).toLocaleString()} more points to{" "}
        ...
        {tier === "diamond" && (
          <p className="text-[11px] text-accent/80 mt-1">
            ✦ Highest tier — all benefits unlocked
          </p>
        )}
      </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            ...
            <div>
              ...
              <div className="font-display text-lg text-text-primary tabular-nums">
                <MoneyText
                  ngn={parseFloat(summary.lifetime_value_ngn || "0")}
                />
              </div>
            </div>
            <div>
      ...
      {latestChurn ? (
        <div className="p-4 rounded-[14px] bg-text-primary/[0.04] border hairline">
          ...
          <Button
            ...
            onClick={() =>
              navigate(`/retention?contact=${contactId}&action=win-back`)
            }
          >
        ...
        <div className="p-4 rounded-[14px] bg-success/[0.06] border border-success/20">
          ...
          <p className="text-[11.5px] text-text-muted mt-1">
            No churn risk score on record — {contactName} appears to be an
            engaged contact.
          </p>
        </div>
      )}
      ...
      <div className="p-4 rounded-[14px] bg-text-primary/[0.04] border hairline">
        ...
        <p className="text-[12px] text-text-muted mb-3">
          Referral history, earnings, and contacts referred by {contactName} are
          tracked in the Referrals module.
        </p>
        <Button
          variant="ghost"
          size="sm"
      ...
      <div className="p-3 rounded-[12px] bg-text-primary/[0.03] border hairline">
        ...
        <p className="text-[11.5px] text-text-faint leading-relaxed">
          Points are earned automatically on every purchase (proportional to
          amount paid). Partial payments earn partial points. Points expire
