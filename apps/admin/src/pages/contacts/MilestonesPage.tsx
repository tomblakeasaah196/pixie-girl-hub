...
const AVATAR_COLORS = [
  "#8b9d77", "#7a8fa8", "#b76e79", "#9c7ad9", "#5aa0a8", "#a8785a",
];
...
export function MilestonesPage() {
  ...
  const thisWeek = filtered.filter((m) => m.days_until > 0 && m.days_until <= 7).length;
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="flex-1">
          <h1 className="font-display text-2xl text-text-primary">Milestones</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          ...
          >
            ...
            <div className={`font-display text-2xl tabular-nums text-${tone}`}>{value}</div>
          </div>
        ))}
      </div>
          ...
          {Object.entries(grouped).map(([month, items]) => (
            <div key={month}>
              ...
              <div className="flex flex-col gap-2">
                {items.map((m) => {
                  const { text: daysText, tone: daysTone } = daysLabel(m.days_until);
                  const EventIcon = EVENT_ICONS[m.event_type] ?? Calendar;
                  const colorIdx = Math.abs(
                    m.display_name.split("").reduce((a, c) => a + c.charCodeAt(0), 0),
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {m.priority_level === "vip" && (
                            <Pill tone={PRIORITY_TONE[m.priority_level]} dot={false}>
                              VIP
                            </Pill>
                          )}
                        </div>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="text-[11px] text-text-faint">
                            {new Date(m.event_date).toLocaleDateString("en-NG", {
                              day: "numeric",
                              month: "short",
                            })}
                          </span>
