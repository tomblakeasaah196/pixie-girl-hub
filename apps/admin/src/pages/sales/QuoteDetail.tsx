...
import { Send, CheckCircle, ArrowRightCircle } from "lucide-react";
...
const SEND_OPTIONS: {
  value: NonNullable<QuotationSendInput["sent_via"]>;
  label: string;
}[] = [
...
export function QuoteDetail({
  ...
  const [sendVia, setSendVia] =
    useState<NonNullable<QuotationSendInput["sent_via"]>>("email");
  ...
  const handleConvert = async () => {
    ...
    await convertQuote.mutateAsync({
      id: quoteId,
      input: { sales_channel: convertChannel },
    });
  ...
  return (
    <>
      <Drawer
        open={!!quoteId}
        onClose={onClose}
        title={quote?.quotation_number ?? "Quotation"}
        subtitle={
              ...
              <FormGrid>
                <div>
                  ...
                  <div className="text-[14px] font-semibold mt-1">
                    {quote.contact_name ?? quote.contact_id.slice(0, 8)}
                  </div>
                </div>
                <div>
                ...
                <div>
                  <div className="micro">Valid Until</div>
                  <div className="text-[13px] mt-1">
                    {quote.valid_until
                ...
                <div>
                  ...
                  <div className="text-[13px] mt-1 capitalize">
                    {quote.delivery_type?.replace(/_/g, " ") ?? "—"}
                  </div>
                </div>
              </FormGrid>
              {quote.payment_terms && (
                <div className="mt-3">
                  ...
                  <div className="text-[13px] mt-1">{quote.payment_terms}</div>
                </div>
              )}
              {quote.notes && (
                <div className="mt-3">
                  <div className="micro">Notes</div>
                  <div className="text-[13px] mt-1 text-text-muted">
                    {quote.notes}
                  </div>
                </div>
              )}
              ...
              <Card className="p-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-[13px]">
                    ...
                    <MoneyText ngn={Number(quote.subtotal_ngn)} />
                  </div>
                  {Number(quote.discount_amount_ngn) > 0 && (
                    <div className="flex justify-between text-[13px]">
                      <span className="text-text-muted">Discount</span>
                      <MoneyText ngn={Number(quote.discount_amount_ngn)} />
                    </div>
                  )}
                  {Number(quote.tax_amount_ngn) > 0 && (
                    <div className="flex justify-between text-[13px]">
                      <span className="text-text-muted">Tax</span>
                      <MoneyText ngn={Number(quote.tax_amount_ngn)} />
                    </div>
                  )}
                  {Number(quote.shipping_fee_ngn) > 0 && (
                    <div className="flex justify-between text-[13px]">
                      <span className="text-text-muted">Shipping</span>
                      <MoneyText ngn={Number(quote.shipping_fee_ngn)} />
                    </div>
                  )}
                  <div className="flex justify-between text-[15px] font-semibold pt-1">
                    <span>Total</span>
                    <MoneyText ngn={Number(quote.total_ngn)} />
                  </div>
                </div>
              </Card>
            )}
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Select
                ...
                onChange={setConvertChannel}
                options={
                  SALES_CHANNELS as unknown as {
                    value: SalesChannel;
                    label: string;
                  }[]
                }
