...
import { Sparkles, ChevronDown, ChevronUp, Loader2, RefreshCw, AlertTriangle } from "lucide-react";
import type { AiDealSummary, AiNextActionsResult, AiChurnExplainer } from "../types";
  ...
  return (
    <div className="rounded-[14px] border border-accent/20 bg-accent/[0.04] overflow-hidden">
      ...
      >
        ...
        <span className="text-[12px] font-semibold text-accent flex-1 text-left">{title}</span>
        {collapsible && (
          open ? <ChevronUp className="w-3.5 h-3.5 text-accent/60" /> : <ChevronDown className="w-3.5 h-3.5 text-accent/60" />
        )}
      </button>
  ...
  if (!loaded && !summary && !isLoading) {
    return (
      ...
      >
        ...
        <span className="text-[12px] font-semibold text-accent">Get AI deal summary</span>
      </button>
    );
  }
    ...
    >
      {summary && (
        <div className="flex flex-col gap-3">
          <p className="text-[12.5px] text-text-primary leading-relaxed">{summary.summary}</p>
            ...
            <div>
              ...
              <ul className="flex flex-col gap-1">
                {summary.key_risks.map((r, i) => (
                  <li key={i} className="flex items-start gap-1.5 text-[11.5px] text-warn/90">
          ...
          <div className="p-2.5 rounded-[10px] bg-accent/[0.08] border border-accent/20">
            ...
            <p className="text-[12px] text-accent-glow">{summary.recommended_next_step}</p>
          </div>

          <div className="flex items-center justify-between text-[10.5px] text-text-faint">
            {summary.win_probability_estimate !== null && (
              <span>Win probability estimate: {summary.win_probability_estimate}%</span>
            )}
  ...
  if (!loaded && !result && !isLoading) {
    return (
      ...
      >
        ...
        <span className="text-[11.5px] font-semibold text-accent">Suggest next actions</span>
      </button>
    );
  }

  const urgencyColor = { high: "text-danger", medium: "text-warn", low: "text-success" } as const;
    ...
    >
      {result && (
        <div className="flex flex-col gap-2">
          {result.reasoning && (
            <p className="text-[11px] text-text-faint leading-relaxed">{result.reasoning}</p>
          )}
          {result.actions.map((action, i) => (
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div>
                  <div className="text-[12px] font-medium text-text-primary">{action.action_type}</div>
                  <p className="text-[11.5px] text-text-muted mt-0.5">{action.description}</p>
...
export function AiChurnExplainerCard({
  ...
  if (!loaded && !explainer && !isLoading) {
    return (
      ...
      >
        ...
        <span className="text-[11.5px] font-semibold text-warn">Explain churn risk</span>
      </button>
    );
  }
  ...
  return (
    ...
    >
      {explainer && (
        <div className="flex flex-col gap-3">
          <div className="text-[12.5px] font-semibold text-warn">{explainer.headline}</div>
          <p className="text-[12px] text-text-muted leading-relaxed">{explainer.explanation}</p>
          ...
          {explainer.win_back_prompt && (
            <div className="p-2.5 rounded-[10px] bg-accent/[0.06] border border-accent/20">
              ...
              <p className="text-[11.5px] text-accent/90 italic">"{explainer.win_back_prompt}"</p>
            </div>
          )}
        </div>
      )}
