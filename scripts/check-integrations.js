...
const checks = [
  ...
  [
    ...
    () =>
      has("TWILIO_ACCOUNT_SID", "TWILIO_AUTH_TOKEN", "TWILIO_FROM")
        ...
        : [
            NO,
            "optional — set TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_FROM",
          ],
  ],
  [
  ...
  [
    ...
    () =>
      enumSet("FX_PROVIDER") && has("FX_API_KEY")
        ...
        : [
            NO,
            "set FX_PROVIDER + FX_API_KEY (the refresh cron no-ops otherwise)",
          ],
  ],
  [
    "Embeddings / RAG",
    () =>
      enumSet("EMBEDDINGS_PROVIDER") && has("EMBEDDINGS_API_KEY")
        ...
        : [
            NO,
            "set EMBEDDINGS_PROVIDER + EMBEDDINGS_API_KEY (Praxis RAG is inert otherwise)",
          ],
  ],
  [
  ...
  [
    ...
    () => {
      const on = ["true", "1"].includes(
        String(env.PRAXIS_ORCHESTRATOR_ENABLED || "").toLowerCase(),
      );
      return on
        ? [
            PASS,
            `enabled; vendor '${env.PRAXIS_LLM_VENDOR || "deepseek"}' creds live in AI Control (DB)`,
          ]
        ...
        return [
          ...
          "puppeteer installed" +
            (env.PUPPETEER_EXECUTABLE_PATH
              ? ` (exec ${env.PUPPETEER_EXECUTABLE_PATH})`
              : ""),
        ];
      } catch {
        return [
          WARN,
          "PDF_ENABLED but puppeteer not installed — run npm install",
        ];
      }
    },
  ],
    ...
    () =>
      has("CHOWDECK_API_KEY") || has("GIGL_API_KEY")
        ? [
            PASS,
            [
              has("CHOWDECK_API_KEY") && "Chowdeck",
              has("GIGL_API_KEY") && "GIGL",
            ]
              .filter(Boolean)
              .join(", "),
          ]
