...
const NAV_TILES = [
  {
    label: "Users & Access",
    desc: "Provision, manage, and review user accounts",
    icon: Users,
    to: "/iam-security/users",
  },
  {
    label: "Audit Log",
    desc: "Append-only trail of every action",
    icon: ScrollText,
    to: "/iam-security/audit",
  },
  {
    label: "Security Events",
    desc: "Logins, password changes, permission updates",
    icon: Activity,
    to: "/iam-security/events",
  },
  {
    label: "Sessions",
    desc: "Active sessions and device management",
    icon: Monitor,
    to: "/iam-security/sessions",
  },
  {
    label: "Access Reviews",
    desc: "Periodic attestation for compliance",
    icon: ClipboardCheck,
    to: "/iam-security/reviews",
  },
  {
    label: "MFA Setup",
    desc: "Configure your multi-factor authentication",
    icon: Shield,
    to: "/iam-security/mfa",
  },
];
...
export function IamSecurityPage() {
  ...
  return (
    <div className="max-w-[1000px] mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-text-muted text-[13px]">
            Identity, access, audit, and compliance
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <span className="flex-1 min-w-0">
                <span className="font-display text-[15px] block">
                  {t.label}
                </span>
      ...
      <div>
        <h3 className="text-[11px] uppercase tracking-wide font-bold text-text-muted mb-3">
          ...
          {!s?.recent_events || s.recent_events.length === 0 ? (
            ...
            <div>
              {s.recent_events.slice(0, 10).map((evt: SecurityEvent) => (
                <div
                  key={evt.log_id}
                  className="flex items-start gap-3 py-3 border-b hairline last:border-0"
                >
      ...
      {/* Failed Logins */}
      <Modal
        open={detailModal === "failed_logins"}
        ...
        {!s?.failed_logins_24h?.length ? (
          ...
          <div className="space-y-2">
            {s.failed_logins_24h.map((u, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-3 rounded-xl bg-text-primary/[0.03] border border-line"
              >
                <div>
      ...
      {/* Inactive Accounts */}
      <Modal
        open={detailModal === "inactive"}
        onClose={() => setDetailModal(null)}
        title="Inactive Accounts (90d+)"
      >
        <div className="text-center py-4">
          <div className="font-display text-[36px] font-medium text-warn mb-2">
            {s?.inactive_accounts ?? 0}
          </div>
          <p className="text-text-muted text-[13px]">
          ...
          <button
            onClick={() => {
              setDetailModal(null);
              navigate("/iam-security/users?status=active");
            }}
      ...
      {/* Locked Accounts */}
      <Modal
        open={detailModal === "locked"}
        onClose={() => setDetailModal(null)}
        title="Locked Accounts"
      >
        <div className="text-center py-4">
          <div className="font-display text-[36px] font-medium text-danger mb-2">
            {s?.locked_accounts ?? 0}
          </div>
          <p className="text-text-muted text-[13px]">
            accounts currently locked due to repeated failed logins
          </p>
          <button
            onClick={() => {
              setDetailModal(null);
              navigate("/iam-security/users?status=locked");
            }}
            className="mt-3 text-[13px] text-accent-glow hover:underline"
          >
            View in Users & Access &rarr;
      ...
      {/* Pending Invites */}
      <Modal
        open={detailModal === "pending_invites"}
        onClose={() => setDetailModal(null)}
      ...
      >
        <div className="text-center py-4">
          ...
          <button
            onClick={() => {
              setDetailModal(null);
              navigate("/iam-security/users?status=invited");
            }}
            className="mt-3 text-[13px] text-accent-glow hover:underline"
          >
            View in Users & Access &rarr;
      ...
      {/* Users Without MFA */}
      <Modal
        open={detailModal === "no_mfa"}
        onClose={() => setDetailModal(null)}
        title="Users Without MFA"
      >
        <div className="text-center py-4">
          <div className="font-display text-[36px] font-medium text-warn mb-2">
            {s?.users_without_mfa ?? 0}
          </div>
          <p className="text-text-muted text-[13px]">
            active users without multi-factor authentication enabled
          </p>
          <button
            onClick={() => {
              setDetailModal(null);
              navigate("/iam-security/mfa");
            }}
            className="mt-3 text-[13px] text-accent-glow hover:underline"
          >
      ...
      {/* Active Sessions */}
      <Modal
        open={detailModal === "sessions"}
        onClose={() => setDetailModal(null)}
        title="Active Sessions"
      >
        <div className="text-center py-4">
          <div className="font-display text-[36px] font-medium text-success mb-2">
            {s?.active_sessions ?? 0}
          </div>
          <p className="text-text-muted text-[13px]">
          ...
          <button
            onClick={() => {
              setDetailModal(null);
              navigate("/iam-security/sessions");
            }}
            className="mt-3 text-[13px] text-accent-glow hover:underline"
          >
            View Sessions &rarr;
