...
export function CommandPalette() {
  ...
  const results = useMemo(() => {
    ...
    const quick = QUICK.filter(
      (a) => !t || a.label.toLowerCase().includes(t),
    ).map((a) => ({ ...a, kind: "quick" as const }));
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[91] bg-black/50 backdrop-blur-[3px] animate-fade-in"
      ...
      <div className="fixed top-[14vh] left-1/2 -translate-x-1/2 w-[min(560px,94vw)] max-h-[66vh] z-[92] rounded-[18px] overflow-hidden flex flex-col dropglass animate-fade-in">
        ...
        <div className="overflow-y-auto p-2">
          {results.length === 0 && (
            <div className="p-6 text-center text-text-faint">
              No matches for “{q}”.
            </div>
          )}
          {results.map((r, i) => {
            ...
            return (
              <div
                ...
                onClick={() => go(i)}
                className={cn(
                  "flex items-center gap-3 p-[10px_12px] rounded-[11px] cursor-pointer",
                  sel === i && "bg-accent/10",
                )}
              >
                ...
                <span className="flex-1">
                  ...
                  <span className="block text-[11px] text-text-faint">
                    {r.kind === "quick" ? `in ${r.where}` : r.where}
                  </span>
                </span>
                {sel === i && (
                  <CornerDownLeft className="w-3.5 h-3.5 text-text-faint" />
                )}
              </div>
            );
          })}
