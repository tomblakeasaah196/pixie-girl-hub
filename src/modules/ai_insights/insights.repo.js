...
function tableFor(brand) {
  ...
  return `${brand}.ai_insight_stock_alerts`;
}
...
async function findById({ client, brand, id }) {
  const sql = `SELECT * FROM ${tableFor(brand)} WHERE ai_insight_stock_alerts_id = $1 LIMIT 1`;
  // NOTE: replace `ai_insight_stock_alerts_id` with the actual PK name for this table
  const exec = client ? client.query.bind(client) : query;
