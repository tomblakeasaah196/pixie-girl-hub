  ...
  return (
    <div className="flex items-center gap-3">
      {units.map((u) => (
        <div
          key={u.l}
          className="min-w-[58px] md:min-w-[68px] rounded-[14px] bg-black/35 backdrop-blur-md border border-white/15 px-3 py-2.5 text-center"
        >
          <div className="font-mono text-[22px] md:text-[28px] tabular-nums text-white leading-none">
...
export function SalesIndexPublic() {
  ...
  useEffect(() => {
    if (brandKey && localStorage.getItem(`pgh-sale-list:${brandKey}`))
      setSubscribed(true);
  }, [brandKey]);

  const upcomingNext = data?.upcoming?.[0];
  const heroImage =
    data?.active?.hero_image_url || upcomingNext?.hero_image_url || null;
    ...
    <div className="bg-bg text-text-primary antialiased min-h-screen">
      ...
      <div className="flex items-center justify-between px-6 md:px-12 py-5">
        ...
        <div className="flex items-center gap-5 text-[12.5px] text-text-muted">
          ...
          <a href="#join" className="text-accent-glow font-semibold">
            Join the list
          </a>
        </div>
      </div>
          ...
          <div className="max-w-[1140px] mx-auto">
            {data?.active ? (
              <div className="max-w-[680px]">
                ...
                <h1 className="font-display text-[clamp(40px,7vw,82px)] leading-[0.98] text-white">
                  {data.active.name}
                </h1>
                {data.active.hero_subtitle && (
                  <p className="mt-5 text-[15px] md:text-[18px] text-white/85 max-w-[540px]">
                    {data.active.hero_subtitle}
                  </p>
                )}
                <div className="mt-8">
                  <CountdownRow to={data.active.ends_at} />
                </div>
                <a
              ...
              <div className="max-w-[720px]">
                <div className="text-[11px] tracking-[0.34em] uppercase text-[#F4E9D9]/80 mb-5">
                  Between drops
                </div>
                <h1 className="font-display text-[clamp(40px,7.5vw,88px)] leading-[0.98] text-white">
                  The next chapter is{" "}
                  <span className="italic text-accent-glow">
                    being prepared.
                  </span>
                </h1>
                <p className="mt-5 text-[15px] md:text-[18px] text-white/85 max-w-[560px]">
                  Our drops are limited, intentional, and gone quickly. Join the
                  list to be first through the door — with private prices and
                  the occasional gift.
                </p>
                {upcomingNext && (
                  <div className="mt-8">
      ...
      <section id="join" className="px-6 md:px-12 py-16 md:py-24">
        <div className="max-w-[640px] mx-auto text-center">
          ...
          <h2 className="font-display text-[30px] md:text-[44px] leading-[1.05]">
            Be first, always
          </h2>
          <p className="text-text-muted mt-4 leading-relaxed">
            Early access before every public launch, private prices, and first
            pick while it lasts. We send rarely — only when it matters.
          </p>
        ...
        <section className="px-6 md:px-12 pb-20">
          <div className="max-w-[1140px] mx-auto">
            <div className="flex items-end justify-between gap-4 mb-8">
              <div>
                <div className="text-[11px] tracking-[0.3em] uppercase text-accent-glow/90 font-semibold mb-2">
                  The archive
                </div>
              ...
              {!subscribed && (
                <a
                  href="#join"
                  className="hidden md:inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-accent-glow"
                >
            ...
            <div className="relative">
              <div
                className={`grid grid-cols-2 lg:grid-cols-3 gap-4 ${subscribed ? "" : "blur-[6px] pointer-events-none select-none"}`}
              >
                {(data?.past || []).map((c) => (
                  <ArchiveCard
                    key={c.slug}
              ...
              {!subscribed && (
                <div className="absolute inset-0 grid place-items-center">
                  ...
                  >
                    <Lock className="w-4 h-4 text-accent-glow" /> Join the list
                    to unlock past drops
                  </a>
                </div>
              )}
            </div>
      ...
      <section className="px-6 md:px-12 pb-20">
        <div className="max-w-[1140px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <CtaCard
            ...
            href={
              data?.active
                ? `/sale/${data.active.slug}${brand ? `?brand=${brand}` : ""}`
                : storeUrl || "#join"
            }
          />
          <CtaCard
            ...
            title="Apply for VIP"
            sub="Our inner circle + ambassadors"
            href="#join"
          />
          <CtaCard
            icon={<Instagram className="w-5 h-5" />}
            title="Follow along"
            sub="Behind every drop"
            href={igUrl || "#"}
          />
        </div>
      </section>
      ...
      <footer className="border-t border-line/60 px-6 md:px-12 py-10">
        <div className="max-w-[1140px] mx-auto flex flex-col md:flex-row gap-4 md:items-center justify-between text-[12px] text-text-faint">
          ...
          <div>
            © {new Date().getFullYear()} {brandName}. By invitation.
          </div>
        </div>
      </footer>
    </div>
...
function ArchiveCard({
  ...
  const inner = (
    <div className="group rounded-[18px] overflow-hidden border border-line/70 bg-panel/40">
      ...
      <div className="p-4">
        ...
        <div className="text-text-faint text-[12px] mt-1">
  ...
  return (
    <a
      href={`/sale/${c.slug}${brand ? `?brand=${brand}` : ""}`}
...
function CtaCard({
  ...
  return (
    ...
    >
      <div className="w-10 h-10 rounded-[12px] bg-accent/[0.12] text-accent-glow grid place-items-center mb-4">
        {icon}
      </div>
...
function JoinForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      await subscribeSalesList(
        { email, phone, first_name: firstName || undefined },
        brand,
      );
      onJoined();
    } catch (err) {
    ...
    return (
      <div className="mt-8 rounded-[18px] border border-success/40 bg-success/[0.08] px-6 py-7">
        ...
        <p className="text-text-muted text-[13.5px] mt-2">
          Watch your inbox and WhatsApp — you'll hear from us before anyone
          else. The archive below is now unlocked.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="mt-8 text-left">
      ...
      <p className="text-text-faint text-[11.5px] mt-3 text-center">
        Early access, private prices and the occasional gift. No noise — leave
        anytime.
      </p>
    </form>
  );
